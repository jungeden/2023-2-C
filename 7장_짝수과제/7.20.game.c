...
int main(void){
    ...
    do
    {
        printf("현재 스틱의 개수 : %d개",s);
        printf("몇개의 스틱을 가져가시겠습니까? : "); scanf("%d",&u);

        if (u>3) printf("스틱은 0개에서 3개사이로 가져갈 수 있습니다.");
        else if (u>s) {
            ...
            else if (a=='N') printf("몇개의 스틱을 가져가시겠습니까? : "); scanf("%d",&u);
            else printf("Y,N 중 선택하세요. 계속 진행하시겠습니까?(Y/N)"); scanf("%c",&a);
        }
        else s=s-u;

        c=rand()%4;
        printf("컴퓨터는 %d개의 스틱을 가져갔습니다.",c);
        if (c)
